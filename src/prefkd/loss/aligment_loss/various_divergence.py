...
class VariousDivergence(CrossEntropyLoss):
    ...
    def forward(
        ...
        log = {}
        loss = self.compute_cross_entropy_loss(outputs.logits, output_data["label"], log=log)[0]
        ...
        with torch.no_grad():
            ...
            teacher_outputs = teacher_model(
                ...
                position_ids=input_data.get("teacher_position_ids", None),
                output_hidden_states=True,
            )
    ...
    def compute_forward_kl_divergence(
        self, logits, teacher_logits, target, reduction="sum", log=None, use_tea_temp=False
    ):
    ...
    def compute_adaptive_kl_divergence(
        ...
        probs = torch.softmax(logits / self.kd_temp, dim=-1, dtype=torch.float32)
        if use_tea_temp:
        ...
        fkl = self.compute_forward_kl_divergence(
            logits, teacher_logits, target, reduction="none", use_tea_temp=use_tea_temp
        )
        rkl = self.compute_reverse_kl_divergence(
            logits, teacher_logits, target, reduction="none", use_tea_temp=use_tea_temp
        )
    ...
    def compute_skewed_forward_kl_divergence(
        ...
        teacher_probs = torch.softmax(teacher_logits, -1, dtype=torch.float32)
        mixed_probs = (
            self.args.skew_lambda * teacher_probs + (1 - self.args.skew_lambda) * student_probs
        )
    ...
    def compute_skewed_reverse_kl_divergence(
        ...
        teacher_probs = torch.softmax(teacher_logits, -1, dtype=torch.float32)
        mixed_probs = (
            1 - self.args.skew_lambda
        ) * teacher_probs + self.args.skew_lambda * student_probs
        mixed_lprobs = torch.log(mixed_probs)
