...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, distiller, input_data, output_data, logging_output, batch_denom):
        ...
        logging_output = self.record_logging_output(
            logging_output, batch_denom, {"loss": loss, "nll_loss": nll_loss, "accuracy": accuracy}
        )
    ...
    def record_logits(self, logits, target, log, teacher_logits=None, teacher_target=None):
        ...
        pos_target = torch.where(pad_mask, torch.zeros_like(target), target)
        ...
        if teacher_logits is not None:
            ...
            pos_teacher_target = torch.where(
                teacher_pad_mask, torch.zeros_like(teacher_target), teacher_target
            )
            teacher_target_logits = teacher_logits.gather(
                -1, pos_teacher_target.unsqueeze(-1)
            ).squeeze(-1)
            teacher_non_target_logits = (teacher_logits.sum(-1) - teacher_target_logits) / (
                teacher_logits.shape[-1] - 1
            )
            teacher_target_logits = teacher_target_logits.masked_fill_(teacher_pad_mask, 0.0).sum()
            teacher_non_target_logits = teacher_non_target_logits.masked_fill_(
                teacher_pad_mask, 0.0
            ).sum()

            log["teacher_target_logits"] = teacher_target_logits
            log["teacher_non_target_logits"] = teacher_non_target_logits

    def record_logging_output(
        self,
        logging_output,
        # batch_denom,
        content,
