...
class DualSpaceKDWithCMA(VariousDivergence):
    ...
    def forward(
        ...
        log = {}
        loss = self.compute_cross_entropy_loss(logits, output_data["label"], log=log)[0]
        ...
        kd_loss, log = self.compute_dual_space_kd_loss_with_cma(
            model_outputs, teacher_model_outputs, input_data, output_data, distiller, log
        )
    ...
    def compute_dual_space_kd_loss_with_cma(
        ...
        teacher_target = output_data["teacher_label"]

        pad_mask = target.ne(self.padding_id)
        teacher_pad_mask = teacher_target.ne(self.padding_id)

        hiddens = outputs["last_hidden_states"]
        teacher_hiddens = teacher_outputs["last_hidden_states"]
        ...
        if hasattr(distiller.teacher_model, "model") and hasattr(
            ...
            distiller.teacher_model.transformer, "wte"
        ):
