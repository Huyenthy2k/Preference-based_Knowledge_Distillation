...
# from prefkd.utils.utils import rank0_print
...
class Distiller(nn.Module):
    def __init__(
        ...
        self.student_model, self.student_tokenizer = student_model, student_tokenizer
        self.student_hidden_size = student_model.config.hidden_size

        # if self.config.teacher_model_path is not None:
        self.teacher_model, self.teacher_tokenizers = teacher_model, teacher_tokenizer
        self.teacher_hidden_size = teacher_model.config.hidden_size
        # else:
        ...
        if self.teacher_model and config.projector_config_path:
            ...
            rank0_print(f"projector structure: {self.projectors}")
        ...
        #     rank0_print(
        #         f"Load teacher-to-student token mapping from {config.teacher_to_student_token_mapping}"
        #     )
        ...
        #     rank0_print(
        #         f"Load teacher-to-student id mapping from {config.teacher_to_student_id_mapping}"
        #     )
    ...
    @staticmethod
    def add_distiller_config(parser):
    ...
    def load_existing_projectors(self):
        ...
        if os.path.exists(projector_path):
            ...
            rank0_print("Existing projector params: {}".format(list(projector_params.keys())))
            for key in self.projectors:
                try:
                    ...
                    rank0_print("Load projector '{}' from current path.".format(key))
                except:
                    rank0_print("Not compatible for projector '{}'".format(key))
                    continue
    ...
    #     else:
    #         rank0_print(' > number of parameters: {:,}'.format(
    ...
    #     rank0_print("Loading teacher model...")
    ...
    #             raise NotImplementedError
    #     else:
    #         rank0_print(' > number of parameters of the teacher model: {:,}'.format(
    #             sum([p.nelement() for p in model.parameters()])
    #         ))
    ...
    def add_optimizer_param_group(self, optimizer):
        if hasattr(self, "projectors"):
            if self.config.projector_lr:
                pretrained_proj = (
                    self.config.pretrained_projector.split(",")
                    if self.config.pretrained_projector is not None
                    else []
                )
    ...
    def forward(
        ...
        input_data["input_ids"] = batch["chosen_student_input_ids"]
        input_data["attention_mask"] = batch["chosen_student_attention_mask"]
        input_data["teacher_input_ids"] = batch["chosen_teacher_input_ids"]
        input_data["teacher_attention_mask"] = batch["chosen_teacher_attention_mask"]

        output_data["label"] = batch["chosen_student_labels"]
        output_data["teacher_label"] = batch["chosen_teacher_labels"]
        loss, logging_output = criterion(
            self,
            input_data=input_data,
            model_outputs=model_outputs,
            teacher_model_outputs=teacher_model_outputs,
